...
public class DateTimeUtil {
    ...
    public static final String YYMMDDHHmmssSSS = "yyyyMMddSSS";
    ...
    public static String dateTimeToStr(DateTime dateTime, String format) {
        return format(LocalDateTime.ofInstant(dateTime.toInstant(), ZoneId.systemDefault()), format);
    }
...
//        return Date.from(zdt.toInstant());
        ...
        //直接用DateTimeUtil转 由于编码问题会报错
        DateTimeFormatter df = DateTimeFormatter.ofPattern(format);
        LocalDateTime parse = LocalDateTime.parse(sDate, df);
     ...
     * @return long
     */
    public static long getDistanceOfTwoDate_long(Date before, Date after) {
        long beforeTime = before.getTime();
        long afterTime = after.getTime();
     ...
     * @return
     * @Author Liwx
     * @Description 时间比较 > 0 时间超出， <0
     * @Date 2023/6/18 12:43
     * @Param
     **/
    public static Integer compareByString(String time1str, String time2str) {
        DateTime time1 = DateTime.of(time1str, DateTimeUtil.DATE_FORMAT_MINUTE);
        DateTime time2 = DateTime.of(time2str, DateTimeUtil.DATE_FORMAT_MINUTE);
        return time1.compareTo(time2);

    }

    public static void main(String[] args) {
        System.out.println(DateTimeUtil.compareByString("2023-05-27 12:10", "2023-05-28 12:30"));
    }

    public static Long getLongByStrDateTime(String oldEndTime) {
        return DateTime.of(oldEndTime, DateTimeUtil.DATE_FORMAT_MINUTE).getTime();
    }
     ...
     * @return
     */
    public static String dateDiffFormat(Date startTime, Date endTime, String format, String str) {
        ...
        if (str.equalsIgnoreCase("dayToMin")) {
            return day + "天" + (hour - day * 24) + "小时" + (min - hour * 60) + "分钟";
        }
        if (str.equalsIgnoreCase("dayToDay")) {
            return day+"";
        }
        return "";
    }
    ...
    public static String getTimeFormat(String time){
        ...
        for (int i = 0; i < time.length(); i++) {
            if (time.charAt(i) == someChar) {
                count++;
            }
        }
        switch (count){
            case 1:
                re=DATE_FORMAT_MINUTE;
                break;
            case 2:
                re= DATE_FORMAT_SECOND;
                break;
            default:
                re=  DATE_FORMAT_DAY;
                break;
